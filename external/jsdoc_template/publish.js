    ...
    publish = function(data, opts, tutorials) {
      ...
      data.orderBy(['longname', 'version', 'since']);
        ...
        function linkto(longname, linktext) {
            var url = '../'+(helper.longnameToUrl[longname] || '').replace('.', '-').toLowerCase().replace('.html', '/');
...
//         }
        ...
        for (var longname in helper.longnameToUrl) {
            if ( hasOwnProp.call(helper.longnameToUrl, longname) ) {
                ...
                if (classes.length) { generate(classes[0].name, classes, helper.longnameToUrl[longname].replace('.', '-').toLowerCase()); }
